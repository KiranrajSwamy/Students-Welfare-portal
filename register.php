...
if((!eregi("[A-Z][a-z]",$n1))||(!eregi("[A-Z][a-z]",$n1))||(!eregi("[A-Z][a-z]",$n1)))
{
 ...
 echo "<a href=r11.html>REGISTER ONCE AGAIN</a>";
}
...
$q=mysql_query("select A_uid from Admin");
while($row=mysql_fetch_array($q))
{
 if($row['A_uid']==$p)
 {
...
if($c==0)
{
$q=mysql_query("select A_id from Admin" );
while($row=mysql_fetch_array($q))
{
 if($row['A_id']==$p)
 {
...
//$a=$stu%4;
mysql_query("insert into Student_(Stud_id,Sch_id,A_id,Sname,S_uid,S_password,Email_id,DOB,Course,Adress,Mob_no,Gender) values('$stu','$s',100,'$n','$p','$cp','$e','$ch','$field','$addr','$mob','$g')",$con);
mysql_query("update Student set Sch_id='$s',A_id='$a where Stud_id='$stu'",$con);
